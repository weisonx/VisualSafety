...
- 在同一界面提供：
  - 强制关机
  - 强制重启
- 快速控制（管理员）：强化防火墙入站、启用/禁用远程桌面、启用/禁用文件共享（SMB）、禁用 SMBv1。
...
- [x] 为所有页加上鼠标悬浮Tips，如一些微软程序说明、常见端口说明等等
- [x] 新增插件页，里面是插件市场，每个卡片一个插件，安装插件后可在左侧页面中显示。每个插件通过本项目发布；定义好 插件的输入、参数 、输出、开发规范。

## 9. 插件规范（初版 / Demo）

本项目内置一个“插件市场”页面，用于展示可用插件与已安装插件。安装后会在左侧导航追加一个插件页面（示例实现），并可通过统一入口运行（演示）。
...
- `version`: 语义化版本
- `author`: 作者/组织
- `icon`: Emoji 图标（统一 EmojiIcons 风格）
- `inputs`: 输入来源说明（例如 `Security.ports` / `Security.controls`）
- `params`: 参数说明（可先用文本描述，后续可扩展 JSON Schema）
- `outputs`: 输出说明（文本/结构化均可）

### 9.2 运行入口

统一运行入口（演示）：`Security.runPlugin(id, params)`，返回一个 `QVariantMap`（在 QML 中可 JSON 展示），同时可写入日志用于审计。
